...
@dataclass(frozen=True)
class SystemAdvisoryEnvelope:
    """
    Immutable non-executable composition of advisory decision context
    for the non-interactive system_principal provenance.

    Sibling of AuthenticatedAdvisoryEnvelope, not a generalization of
    it -- authority here is a SystemPrincipalProvenance, never an
    AuthorityProvenance. Grants no authority, is not an authorization
    decision and is not executable intent.
    """

    decision: OrchestrationDecision
    plan: AdvisorySkillBindingPlan
    authority: SystemPrincipalProvenance
    ...
    def __post_init__(self) -> None:
        ...
        if not isinstance(
            ...
            raise TypeError(
                "authority must be a SystemPrincipalProvenance."
            )
